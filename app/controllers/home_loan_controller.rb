class HomeLoanController < ApplicationController
  ...
  def show
    ...
    when "step2"
      @home_loan = id.nil? ? HomeLoan.new : get_home_loan(id)
      return redirect_to home_loan_path("step1") if id.nil?
    when "step3", "step4"
      @home_loan = id.nil? ? HomeLoan.new : get_home_loan(id)
  ...
  def select_bank
    create_update_home_loan(@update_status_ncl, "Home Loan updated successfully.", home_loan_path("step4"))
  end
  ...
  def update_address
    create_update_home_loan(@update_status_nclo, "Home Loan updated successfully.", home_loan_path("step5"))
  end

  def update_home_assets
    session[:home_loan_id] = nil if @update_status_ncl
    create_update_home_loan(@update_status_ncl, "Home Loan updated successfully.", home_loan_path("step6"))
  end
  ...
  def is_preapproved
    pre_approved_offer = PreApprovedOffer.find_by(mobile_no: @home_loan.mobile_number)
    if @home_loan.mobile_number != pre_approved_offer&.mobile_no
      pre_approved_offer = PreApprovedOffer.new(mobile_no: @home_loan.mobile_number,
        reference_number: "PREAPRO#{(rand*100000000).to_i}",
        first_name: @home_loan.full_name,
        last_name: @home_loan.full_name)
